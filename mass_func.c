...
main()
{
  ...
  read_fof("halo_catalogue25.0",1,&output_flag,&totcluster,halo,&vaa);
  halo= allocate_float_2d(totcluster,7);
  read_fof("halo_catalogue25.0",2,&output_flag,&totcluster,halo,&vaa);
  ...
  for(i=0; i<totcluster; i++)
    halo[i][0]= log10(halo[i][0]*DM_m)+ 10.0; /* masses are converted from 10^10*M_sun*h^-1 unit to M_sun*h^-1 unit*/
  ...
  /*first columb is the (M in h^-1 M_sun) bin position (in x-axis), second columb is the no. of halo per unit vol per unit mass range dn/d(lnM)(in y-axis)[we devide by log(10)=ln(10) to convert dn/d(logM) to dn/d(lnM)]*/
